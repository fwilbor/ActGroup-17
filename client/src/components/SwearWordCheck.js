...
function SwearWordCheck(msg){
  console.log(msg.message)
   ...
   for (const wlist of wlists) {
        if (msg.message.toLowerCase().includes(wlist.toLowerCase())) {
          const tchars = [' ', '!', '.', '@', '$'];
          for (const tchar of tchars) {
            if (msg.message.toLowerCase().includes(wlist.toLowerCase()+tchar) || msg.toLowerCase().includes(tchar+wlist.toLowerCase()) || msg.length === wlist.length) {
            return "Yes";
          }
         }     
        }
